...
flags.DEFINE_boolean('use_tf_vis', True, 'whether to tensorflow visualizer')
...
class Trainer():
    def __init__(self, opts):
        ...
        print("THE SAVE DIR IS:",self.save_dir)
    ...
    def filter_dict(self, state_dict, module_name, empty_dict):
        len_mod = len(module_name)+1
        pretrained_dict = {k[len_mod:]: state_dict[k] for k in state_dict.keys() if k.startswith(module_name)}
        empty_dict.update(pretrained_dict)
        return empty_dict
        
    # helper loading function that can be used by subclasses
    def load_network(self, network, network_label, epoch_label, network_dir=None, module_name=None):
        print('Loading model')
        save_filename = '{}_net_{}.pth'.format(network_label, epoch_label)
        ...
        if module_name is not None:
            ...
            empty_dict = network.state_dict()
            load_dict = self.filter_dict(state_dict, module_name, empty_dict)
