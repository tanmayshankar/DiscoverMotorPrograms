...
class Visualizer():
    ...
    def plot_current_points(self, points, disp_offset=10):
        ...
        for label, pts in points.items():
            ...
            if pts.shape[1] > 3:
                colors = (pts[:, -3:]*255).astype(np.int)
                pts = pts[:, 0:-3]
                self.vis.scatter(
                    pts, opts=dict(title=label, markersize=2, markercolor=colors), win=self.display_id + idx)
            else:
                self.vis.scatter(
                    pts, opts=dict(title=label, markersize=2), win=self.display_id + idx)
            idx += 1
